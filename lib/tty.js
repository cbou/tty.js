...
Server.prototype.handleConnection = function(socket) {
  ...
  socket.on('create', function(cols, rows, func) {
    return socketSession.handleCreate(cols, rows, func);
  });
...
Session.prototype.handleCreate = function(cols, rows, func) {
  ...
  var shell = (typeof conf.shellFn === 'function') ? conf.shellFn(socket) : conf.shell;
  var shellArgs = (typeof conf.shellArgsFn === 'function') ? conf.shellArgsFn(socket) : conf.shellArgs;
