...
Server.prototype.handleConnection = function(socket) {
  var conf = this.conf;
  var socketSession = new Session(this, socket);

  if (typeof sessionStore === 'object') {
    var cookie_string = socketSession.req.headers.cookie;
    var parsed_cookies = parseCookie(cookie_string);
    var connect_sid = parsed_cookies['connect.sid'];
    var session_id = connect_sid.replace('s:', '').replace(/\..*/, '');

    if (session_id) {
      sessionStore.get(session_id, function (error, session) {
        socketSession.expressSession = session;
      });
    }
  }
  ...
  socket.on('create', function(cols, rows, options, func) {
    async.series({
      shell: function(callback){
          if (typeof conf.shellFn === 'function') {
            conf.shellFn(socketSession, options, callback);
          } else {
            callback(null, conf.shell);
          }
      },
      shellArgs: function(callback){
          if (typeof conf.shellArgsFn === 'function') {
            conf.shellArgsFn(socketSession, options, callback);
          } else {
            callback(null, conf.shellArgs);
          }
      },
    },
    // optional callback
    function(err, results){
      return socketSession.handleCreate(cols, rows, results.shell, results.shellArgs, func);
    });
    //var shell = (typeof conf.shellFn === 'function') ? conf.shellFn(socket, options) : conf.shell;
    //var shellArgs = (typeof conf.shellArgsFn === 'function') ? conf.shellArgsFn(socket, options) : conf.shellArgs;
  });

  socket.on('data', function(id, data) {
    return socketSession.handleData(id, data);
  });

  socket.on('kill', function(id) {
    return socketSession.handleKill(id);
  });

  socket.on('resize', function(id, cols, rows) {
    return socketSession.handleResize(id, cols, rows);
  });

  socket.on('process', function(id, func) {
    return socketSession.handleProcess(id, func);
  });

  socket.on('disconnect', function() {
    return socketSession.handleDisconnect();
  });
};
...
Session.prototype.handleCreate = function(cols, rows, shell, shellArgs, func) {
  var self = this;
  var terms = this.terms;
  var conf = this.server.conf;
